...
export const useMiningStore = create<MiningState>((set, get) => ({
  ...
  isMiningAllowed: localStorage.getItem("isMiningAllowed") === "true", // Get mining status from localStorage
  miningCooldown: 0,
  lastMined: Number(localStorage.getItem("lastMined")) || null,

  startMining: () => {
    const currentTime = Math.floor(Date.now() / 1000); // Current time in seconds
    localStorage.setItem("lastMined", currentTime.toString());
    localStorage.setItem("isMiningAllowed", "false");
  ...
  addPoints: (points: number) => {
    ...
    localStorage.setItem("points", newPoints.toString()); // Persist points
  ...
  resetCooldown: () => {
    ...
    localStorage.removeItem("lastMined"); // Clear last mined time
    localStorage.setItem("isMiningAllowed", "true");
  },

  checkMiningStatus: () => {
    const lastMined = localStorage.getItem("lastMined");
    const isMiningAllowed = localStorage.getItem("isMiningAllowed") === "true";
